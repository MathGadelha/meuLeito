...
const DialogCreatePessoaForm = ({
	...
	async function onSubmit(data: z.infer<typeof FormSchemaPessoa>) {
		const params = {
			...data,
			data_nascimento: data.data_nascimento.replace(/(\d{2})\/(\d{2})\/(\d{4})/, "$3-$2-$1"),
	...
	return (
		<Dialog>
			<DialogTrigger asChild ref={triggerDialogRef}>
				<Button variant={"default"} className={`${buttonStyle ? buttonStyle : "rounded-xl"}`} ><PiPlus size={15} className="mr-1" />Criar pessoa</Button>
			</DialogTrigger>
			<DialogContent className="sm:max-w-4xl">
				<DialogHeader>
					<DialogTitle>Criar Pessoas</DialogTitle>
					<DialogDescription>Insira as informações abaixo para criar uma nova pessoa.</DialogDescription>
				</DialogHeader>
