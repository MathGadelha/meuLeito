...
export class VideoSubscriber extends LitElement {
  ...
  firstUpdated() {
    if (this.stream) {
      ...
      this.subscriber = this.session.subscribe(this.stream, document.getElementById(this.stream.streamId), subscriberOptions, (error) => {
        if(error){
          const options = {
            detail: {
              error
  ...
  render() {
    return this.stream
      ? html`<div id=${this.stream.streamId} class='OTSubscriberContainer'><slot></slot></div>`
      : html`Loading`;
  }
}
